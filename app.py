...
def load_models():
    ...
    try:
        return joblib.load("model_terbaik.joblib")
...
for sektor, info in model_dashboard.items():
    st.write(
        sektor,
        "| Skenario:", info.get("skenario"),
        "| PCA:", "pca" in info,
        "| PCA Scaler:", "pca_scaler" in info,
        "| Fitur PCA:", "fitur_pca_input" in info
    )
...
# ============================================================
    ...
    for fitur in official_features:
        ...
        petunjuk_data.append({
            ...
            "Keterangan":
                f"Nilai indikator {fitur} pada bulan tersebut."
        })
            ...
            for sektor in urutan_sektor:
                ...
                try:
                    ...
                    # -----------------------------------------
                        ...
                        # File model saat ini belum menyimpan
                        # scaler PCA dan objek PCA.
                        if pca is None or pca_scaler is None:
                            raise ValueError(
                                f"{skenario} membutuhkan objek "
                                "PCA dan scaler PCA yang tersimpan dalam model "
                                
                            )
                        
                        if fitur_pca_input is None:
                            raise ValueError(
                                f"Fitur input PCA untuk {skenario} tidak ditemukan."
                            )
                        ...
                        if len(fitur_hilang) > 0:
                            raise ValueError(
                                "Fitur Google Trends untuk PCA tidak ditemukan dalam file Excek :"
                                + ",".join(fitur_hilang)
                            )
                        ...
                        if skenario == "S3":
                            ...
                            X_input = pd.concat(
                                [
                                    X_pca,
                                    df[official_features]
                                ],
                                axis = 1
                            )
                            
                            X_input = X_input[fitur].copy()
